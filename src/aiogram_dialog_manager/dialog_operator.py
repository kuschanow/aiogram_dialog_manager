...
class DialogOperator:
    ...
    async def rollback(self, index: int, delete_nodes: bool = False, delete_messages: bool = False, preserve_data: bool = False) -> None:
        if delete_nodes:
            ...
            self._dialog.rollback(index, preserve_data=preserve_data)
            self._dialog.delete_node(node_id_to_delete, preserve_data=preserve_data)
        else:
            self._dialog.rollback(index, preserve_data=preserve_data)

    async def delete_node(self, node_id: str, delete_messages: bool = False, preserve_data: bool = False) -> None:
        if delete_messages:
            await self._delete_messages_in_subtree(node_id)
        self._dialog.delete_node(node_id, preserve_data=preserve_data)
    ...
    async def delete_message(self, message_record: AnyMessageRecord, delete_node: bool = False, delete_messages: bool = False, preserve_data: bool = False) -> None:
        tg = message_record.telegram_message_instance
        await self._bot.delete_message(chat_id=tg.chat.id, message_id=tg.message_id)
        if delete_node:
            for nid, n in self._dialog.nodes.items():
                if n.message is message_record:
                    await self.delete_node(nid, delete_messages=delete_messages, preserve_data=preserve_data)
                    break

    async def delete_all_messages(self, only_current_branch: bool = False, delete_nodes: bool = False, preserve_data: bool = False) -> None:
        messages = (
        ...
        if delete_nodes:
            if only_current_branch:
                ...
                self._dialog.clear_all_nodes(preserve_data=preserve_data)
