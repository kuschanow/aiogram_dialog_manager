...
class VideoExtraParams(BaseModel):
    ...
    cover: Optional[Union[str, InputFile]] = Field(None, description="Cover for the video in the message.")
    start_timestamp: Optional[Union[datetime, timedelta, int]] = Field(None, description="Start timestamp for the video.")
...
class VideoMessagePrototype(InputMediaPrototype, ABC):
    ...
    async def get_input_media(self, dialog, context: Optional[dict[str, Any]], parse_mode=None) -> InputMediaVideo:
        ...
        return InputMediaVideo(
            media=await self.get_video(dialog, context),
    ...
    async def _do_send(
        ...
        video = await self.get_video(dialog, context)
        extra = await self.get_extra_params(dialog, context)
