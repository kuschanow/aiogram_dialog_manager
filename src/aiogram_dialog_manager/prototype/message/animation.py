...
class AnimationMessagePrototype(InputMediaPrototype, ABC):
    @abstractmethod
    async def get_animation(self, dialog, context: Optional[dict[str, Any]]) -> Union[str, InputFile]:
        pass

    async def get_extra_params(self, dialog, context: Optional[dict[str, Any]]) -> AnimationExtraParams:
        return AnimationExtraParams()

    async def get_input_media(self, dialog, context: Optional[dict[str, Any]], parse_mode=None) -> InputMediaAnimation:
